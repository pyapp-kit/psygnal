...
def test_debounced() -> None:
    mock1 = Mock()
    f1 = debounced(mock1, timeout=5, leading=False)
    f2 = Mock()
...
def test_debounced_leading() -> None:
    mock1 = Mock()
    f1 = debounced(mock1, timeout=5, leading=True)
    f2 = Mock()
...
def test_throttled() -> None:
    mock1 = Mock()
    f1 = throttled(mock1, timeout=5, leading=True)
    f2 = Mock()
...
def test_throttled_trailing() -> None:
    mock1 = Mock()
    f1 = throttled(mock1, timeout=5, leading=False)
    f2 = Mock()
